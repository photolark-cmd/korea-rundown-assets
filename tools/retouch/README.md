...
화면 왼쪽 위에 있습니다. 창은 브라우저가 아니라 **윈도 기본 열기 창**으로 뜹니다.

| 버튼 | 쓸 때 |
|---|---|
| **📁 폴더 열기** | 한 폴더를 통째로 필름스트립에 올린다 |
| **🖼 사진 고르기** | 폴더 안에서 **여러 장만 골라서** 올린다 (Ctrl·Shift 로 다중 선택) |
| **＋** | 다른 곳의 사진 한 장을 업로드해서 연다 |

촬영본이 `촬영폴더/원본/학사모` 처럼 여러 겹으로 정리돼 있으면, 상위 폴더를 열었을 때
필름스트립이 **폴더 목록(사진 장수 포함)** 으로 바뀝니다. 눌러서 내려가고 `⬆ 상위 폴더`
로 돌아옵니다. 장수는 하위까지 통틀어 셉니다 — 안쪽에만 사진이 있는 폴더도 보여야 하니까요.
...
| "배경지 바깥 채워" | `extend_backdrop` — 스탠드·바닥·벽이 보이는 곳을 배경지 질감으로 채워 프레임 끝까지. 채울 곳을 다각형으로 대략 지정(Claude가 보고 지정), 경계는 색으로 다듬음 |
| "학사모 사진 점검" | `pose_check` — 고개·모자 윗선·어깨선 기울기를 잰다 (모자는 세그멘테이션 'others' 클래스, 어깨는 옷 마스크 윗선) |
...
실제 학사모 사진 한 장으로 검증: 모자 +2.7° → 수평, 고개 +0.6° → 0.0°, 어깨선 −4.9° → −0.5°.
같은 도구로 **보정사가 완성한 사진**을 재면 모자 −0.3° · 어깨 +0.1° — 측정 기준이 실제 작업 기준과 맞습니다.
성인 프로필(모자 없음, 배경지가 프레임을 못 채움)에서도 고개 0.0° · 어깨선 −0.4°, `extend_backdrop` 으로 배경 확장까지 확인.
