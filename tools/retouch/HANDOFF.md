...
- **사진 고르기**: 툴바의 `📁 폴더 열기` · `🖼 사진 고르기(여러 장)` 가 윈도 기본 열기 창을
  띄운다(`Session.browse`, tkinter). 워커 스레드에서 Tk 를 만들어도 되는 것은 실측 확인함.
  여러 장을 고르면 그 목록이 폴더 목록을 이긴다(`Session.picked` → `listing()` · `path_of()`).
- **썸네일**: 원본이 3~12MB 라 매번 전체 디코딩하면 못 쓴다 → `IMREAD_REDUCED_COLOR_4`
  로 1/4 디코딩 + 메모리 캐시 + 화면에 들어올 때만 요청(IntersectionObserver).
  실측: 첫 요청 0.10초, 캐시 0.003초. 학사모 713장 전수 검사 실패 0.
- **폴더 탐색**: 상위 폴더를 골랐을 때 필름스트립이 하위 폴더 칩으로 바뀐다.
  장수는 **재귀로** 센다 — 직속 사진만 세면 안쪽에만 사진이 있는 촬영 폴더가 목록에서
  통째로 사라진다(월정초에서 실제로 그랬다).

### 사용자의 촬영본 구조 (2026-09-10 실측, 사진은 저장소 밖)

`S:\2021년 졸업사진\[[이가앨범출력]]` 아래 **촬영 30건 · 이미지 29,314장 · 이미지 폴더 556개.**

```
2021.09.16 월정초병설유치원(강)/
  원본/     학사모/ (713장)  프로필/ (689장)
  수정완료/  (144장)   ← 보정사 완성본
  출력/     (141장)
```

**여기가 학습 데이터다.** `원본/` 과 `수정완료/` 가 같은 촬영의 전·후 쌍이므로,
HANDOFF 가 말한 "원본/보정본 쌍 20개"는 이미 채워져 있고도 남는다. 다만 파일명이
1:1 로 맞는지, 수정완료가 원본의 어느 컷인지는 아직 대조 안 했다 — 그게 다음 확인 항목.
...
1. **학사모 원본 5~10장에 `pose_report()` 일괄** → 검출 깨지는 사진 찾기
   (`_hat_mask`, `shoulder_points`). 대상은 위 구조의 `원본/학사모/` — 여러 촬영에서
   골라야 아이·모자가 달라진다. 사람 사진은 저장소 밖 원래 자리에서만 읽고,
   커밋도 업로드도 하지 않는다.
2. **`원본/` ↔ `수정완료/` 파일명 대조.** 쌍이 성립하면 색 LUT 학습(`learn-preset.mjs`)과
   정렬 수치 학습(`prepare.py`)에 바로 들어갈 수 있다.
3. 알려진 약점 1(`extend_backdrop` 자동 영역 탐색)·3(모자 검출 일반화) 손보기.
   둘 다 실사진 여러 장이 있어야 판정된다 → 1번 결과를 보고 정한다.
4. 원본/보정본 쌍이 확인되면 `learn-preset.mjs`(색 LUT) + `prepare.py`(정렬 수치) 부터.
