...
**인물 사진 일괄 보정(잡티·피부·얼굴형)은 `tools/retouch/`** — Python, 사용자 PC의
3080 Ti에서 학습·적용한다. 사용자가 원본/보정본 쌍 몇백 장을 줄 수 있다고 했고,
보정에는 얼굴형 변형까지 들어간다. **사람 사진은 이 public 저장소에 올리지 않는다.**
합성 얼굴로 배관만 검증된 상태이며 실제 쌍으로는 아직 학습 전이다.
한 장씩 작업할 때는 `tools/retouch/studio.py` — 앱 안에 Claude 대화창이 있고 지시를 도구
호출(잡티 힐링·색 조정·피부·얼굴 모델·크롭·저장)로 실행한다. `ANTHROPIC_API_KEY` 필요.
